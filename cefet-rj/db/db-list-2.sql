...
SELECT f.nome AS fornecedor, CONCAT(c.nome, '/', uf.sigla) AS 'cidade/uf'
FROM fornecedor f
...
SELECT cl.nome AS cliente,
    ...
    CONCAT(
        SUBSTRING(v.data_venda, 6, 2), '/', SUBSTRING(v.data_venda, 1, 4)
    ) AS 'mês/ano', c.nome AS cidade
FROM cliente cl
    JOIN bairro b ON (cl.bairro_id = b.id)
    JOIN cidade c ON (b.cidade_id = c.id)
WHERE
...
SELECT p.descricao AS produto, p.preco_custo AS 'preço-custo', ROUND(
        (p.preco_custo * 1.05) - p.preco_custo
    ) AS lucro
FROM produto p
WHERE (
        (p.preco_custo * 1.05) - p.preco_custo
