...
SELECT descricao, endereco FROM fornecedores 
WHERE estado='SP' AND cidade IN('DIADEMA', 'BARUERI');

-- 2)Listar todos os campos de todos os fornecedores do estado SP, exceto os das cidades de BROTAS e SAO PAULO
SELECT * FROM fornecedores 
WHERE estado='SP' AND NOT cidade IN('BROTAS', 'SÃO PAULO');
...
SELECT descricao, telefone FROM fornecedores 
WHERE estado IN('MG', 'SP') AND NOT (cidade='' AND cidade IS NULL);
...
SELECT descricao, pcusto, estoque, ROUND((pcusto*1.25)*estoque, 2) AS balanco FROM produtos;